...
export const HISTORY_GENERAL_PATHWAY: InterviewPathway = {
  ...
  questions: [
    makeQuestion({
      ...
      rationale:
        "Past medical history determines which current symptoms matter most.",
    }),
    makeQuestion({
      ...
      rationale:
        "Smoking is asked separately from chewing because the risks differ.",
    }),
    makeQuestion({
      ...
      options: [
        {
          key: "q.history.tobacco_chewing.opt.never",
          conceptCodes: ["MK-HIS-002"],
        },
        {
          key: "q.history.tobacco_chewing.opt.current",
          conceptCodes: ["MK-HIS-002"],
        },
        { key: "q.history.tobacco_chewing.opt.ex" },
      ],
      rationale:
        "Chewing gets its own question: a combined question reliably loses it.",
    }),
    makeQuestion({
    ...
    makeQuestion({
      ...
      required: false,
      rationale:
        "Allergen and reaction details, asked only when an allergy was reported.",
    }),
    makeQuestion({
      key: "q.history.ros",
      kind: "MULTI_CHOICE",
      ...
      options: [
        ...
        {
          key: "q.history.ros.opt.breathlessness",
          conceptCodes: ["MK-SYM-002"],
        },
        { key: "q.history.ros.opt.none" },
      ],
      rationale:
        "A short review of systems catches complaints the patient did not volunteer.",
    }),
    makeQuestion({
      key: "q.history.notes",
      kind: "FREE_TEXT",
  ...
  branches: [
    {
      ...
      description:
        "Allergy details are asked only when the patient reported an allergy.",
    },
  ],
  escalation: [],
};
