...
export function describeTrigger(expression: TriggerExpression): string {
  ...
  if ("all" in expression)
    return `ALL OF (${expression.all.map(describeTrigger).join("; ")})`;
  if ("any" in expression)
    return `ANY OF (${expression.any.map(describeTrigger).join("; ")})`;
  if ("hasSymptom" in expression)
    return `complaint is ${expression.hasSymptom}`;
  if ("hasSymptomAny" in expression) {
    return `complaint is any of ${expression.hasSymptomAny.join(" | ")}`;
  }
  if ("answeredYes" in expression)
    return `${expression.answeredYes} answered YES`;
  ...
  if ("ageAtLeast" in expression)
    return `age >= ${expression.ageAtLeast} years`;
  ...
  if ("hasCondition" in expression)
    return `history of ${expression.hasCondition}`;
  if ("hasMedication" in expression)
    return `taking ${expression.hasMedication}`;
  if ("hasAllergyCategory" in expression)
    return `allergy of type ${expression.hasAllergyCategory}`;
  ...
  if ("labFlaggedHigh" in expression)
    return `${expression.labFlaggedHigh} flagged high`;
  if ("labFlaggedLow" in expression)
    return `${expression.labFlaggedLow} flagged low`;
...
export function triggerNodeCount(expression: TriggerExpression): number {
  ...
  if ("all" in expression) {
    return (
      1 +
      expression.all.reduce((sum, child) => sum + triggerNodeCount(child), 0)
    );
  }
  if ("any" in expression) {
    return (
      1 +
      expression.any.reduce((sum, child) => sum + triggerNodeCount(child), 0)
    );
  }
  return 1;
}
