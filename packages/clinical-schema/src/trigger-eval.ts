...
export function evaluateTrigger(
  ...
  if ("all" in expression)
    return expression.all.every((child) => evaluateTrigger(child, context));
  if ("any" in expression)
    return expression.any.some((child) => evaluateTrigger(child, context));

  if ("hasSymptom" in expression)
    return context.symptomCodes.includes(expression.hasSymptom);
  if ("hasSymptomAny" in expression) {
    return expression.hasSymptomAny.some((code) =>
      context.symptomCodes.includes(code),
    );
  }
  if ("answeredYes" in expression)
    return context.answeredYes[expression.answeredYes] === true;
  if ("answeredNo" in expression)
    return context.answeredNo[expression.answeredNo] === true;
  if ("answeredAny" in expression)
    return context.answeredAny[expression.answeredAny] === true;
  if ("unanswered" in expression)
    return context.unanswered[expression.unanswered] === true;

  if ("ageAtLeast" in expression) {
    return (
      context.ageYears !== undefined &&
      context.ageYears >= expression.ageAtLeast
    );
  }
  if ("ageAtMost" in expression) {
    return (
      context.ageYears !== undefined && context.ageYears <= expression.ageAtMost
    );
  }
  if ("sex" in expression) return context.sex === expression.sex;
  if ("pregnant" in expression) return context.pregnant === true;

  if ("hasCondition" in expression)
    return context.conditionCodes.includes(expression.hasCondition);
 ...
 */
export function triggerReferences(
  expression: TriggerExpression,
  fact: string,
): boolean {
