...
export interface LabTestDefinition {
  ...
  readonly specimen: 'BLOOD' | 'URINE' | 'STOOL' | 'SPUTUM' | 'SWAB' | 'IMAGING' | 'OTHER';
  readonly canonicalUnit: string;
  readonly alternativeUnits: readonly { readonly unit: string; readonly factor: number }[];
  /**
...
export const LAB_TEST_DEFINITIONS: readonly LabTestDefinition[] = [
  {
    ...
    standardCoding: [{ system: LOINC, code: '718-7', display: 'Hemoglobin [Mass/volume] in Blood' }],
  },
  {
    code: 'MK-LAB-002',
    ...
    plausibleMax: 900,
    precision: 0,
    aliases: ['fasting glucose', 'fasting blood sugar', 'fbs', 'fpg', 'उपवास शर्करा'],
    standardCoding: [
      {
        system: LOINC,
        code: '1558-6',
  ...
  },
  {
    ...
    standardCoding: [
      { system: LOINC, code: '2951-2', display: 'Sodium [Moles/volume] in Serum or Plasma' },
    ],
  },
  {
    code: 'MK-LAB-007',
  ...
  },
  {
    ...
    plausibleMax: 200,
    precision: 1,
    aliases: ['total leucocyte count', 'total leukocyte count', 'tlc', 'wbc', 'श्वेत रक्त कोशिका'],
    standardCoding: [{ system: LOINC, code: '6690-2', display: 'Leukocytes [#/volume] in Blood' }],
  },
  {
    code: 'MK-LAB-009',
    ...
    standardCoding: [{ system: LOINC, code: '777-3', display: 'Platelets [#/volume] in Blood' }],
  },
  {
    code: 'MK-LAB-010',
    display: 'Serum TSH',
    ...
    standardCoding: [
      { system: LOINC, code: '3016-3', display: 'Thyrotropin [Units/volume] in Serum or Plasma' },
    ],
  },
  {
    code: 'MK-LAB-011',
...
export function convertLabToCanonical(
  ...
  const conversion = definition.alternativeUnits.find(
    (candidate) => candidate.unit.toLowerCase() === fromUnit.trim().toLowerCase(),
  );
  if (!conversion) return undefined;
  return { value: value * conversion.factor, unit: definition.canonicalUnit };
}

export function isPlausibleLabValue(labCode: string, canonicalValue: number): boolean {
  const definition = LAB_TEST_BY_CODE.get(labCode);
