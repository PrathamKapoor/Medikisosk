...
import { capabilitiesFor, type AppConfig } from './config/env-with-capabilities';
...
export async function buildApp(deps: BuildAppDeps): Promise<FastifyInstance> {
  ...
  const app = Fastify({
    ...
    trustProxy: true, // The API runs behind a reverse proxy; without this, client IPs are wrong.
    bodyLimit: 1024 * 1024,
  ...
  await app.register(cors, {
    ...
    allowedHeaders: ['Content-Type', 'Authorization', 'Idempotency-Key', 'X-Kiosk-Id', 'X-Kiosk-Token'],
