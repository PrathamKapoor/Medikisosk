...
const envSchema = z.object({
  ...
  MEDIKIOSK_DEPLOYMENT_MODE: z.enum(['local', 'staging', 'production']).default('local'),
  ...
  MEDIKIOSK_SQLITE_PATH: z.string().min(1).default('./.medikiosk-data/medikiosk.sqlite'),
  ...
  MEDIKIOSK_SESSION_TTL_MINUTES: z.coerce.number().int().min(5).max(480).default(45),
  MEDIKIOSK_TEMP_RETENTION_MINUTES: z.coerce.number().int().min(10).max(10080).default(120),
  ...
  LLM_PROVIDER: z.enum(['mock', 'openai', 'ollama', 'disabled']).default('mock'),
  ...
  ASR_PROVIDER: z.enum(['browser', 'bhashini', 'mock', 'disabled']).default('browser'),
  ...
  NER_PROVIDER: z.enum(['deterministic', 'llm', 'disabled']).default('deterministic'),
...
export type AppConfig = Omit<z.output<typeof envSchema>, 'LOG_PHI' | 'METRICS_ENABLED'> & {
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): {
  ...
  const isProduction =
    raw.NODE_ENV === 'production' || raw.MEDIKIOSK_DEPLOYMENT_MODE === 'production';
  ...
  if (raw.MEDIKIOSK_DB_DIALECT === 'postgres' && !raw.DATABASE_URL) {
    throw new Error('Refusing to start: postgres dialect selected but DATABASE_URL is not set.');
  }

  if (raw.IDENTITY_PROVIDER === 'abha' && (!raw.ABDM_CLIENT_ID || !raw.ABDM_CLIENT_SECRET)) {
    throw new Error(
  ...
  if (raw.IDENTITY_PROVIDER === 'mock') {
    warnings.push('Identity uses the deterministic mock provider. This is NOT ABHA.');
  }
  if (raw.LLM_PROVIDER === 'mock') {
    warnings.push('LLM uses the deterministic mock provider. Summaries are template-generated.');
  }
  if (raw.OCR_PROVIDER === 'mock') {
    warnings.push('OCR uses the deterministic synthetic provider. Handwriting is not supported.');
  }
  ...
  const config: AppConfig = {
    ...
    FEATURE_DOCUMENT_AI_ENABLED: readFeatureFlag(env, 'FEATURE_DOCUMENT_AI_ENABLED'),
    ...
    FEATURE_LOCAL_LLM_ENABLED: readFeatureFlag(env, 'FEATURE_LOCAL_LLM_ENABLED'),
    FEATURE_RESEARCH_MODE_ENABLED: readFeatureFlag(env, 'FEATURE_RESEARCH_MODE_ENABLED'),
