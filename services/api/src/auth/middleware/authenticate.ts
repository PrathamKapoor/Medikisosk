...
import { bearerToken, verifyToken, type StaffTokenClaims } from '@medikiosk/auth';
...
export async function authenticateStaff(
  ...
  const result = await verifyToken<StaffTokenClaims>(token, config.MEDIKIOSK_JWT_SECRET, 'STAFF');
  if (!result.ok) {
